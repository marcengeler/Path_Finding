      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			bool current_lane_change = (car_d > 2.5 && car_d < 5.5) || (car_d > 6.5 && car_d < 9.5);
			...
			for (int i = 0; i < sensor_fusion.size(); i++) {
				...
				if (too_close == true) {
					...
					if (lane == 1) {
						if ( d > 0 && d < 4 ) {
							// check if there is a car within +/- reference distance
							car_to_left |= (( check_car_s >  end_path_s - 40) && ((check_car_s - end_path_s) < ref_dist + 10));
						} else if ( d > 8 && d < 12 ) {
							// check if there is a car within +/- reference distance
							car_to_right |= (( check_car_s >  end_path_s - 40) && ((check_car_s - end_path_s) < ref_dist + 10));
						}
					}
