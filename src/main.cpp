      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
				...
				if (too_close == true) {
					if (lane == 2) {
						if ( d > 4 && d < 8 ) {
							...
							car_to_left |= (( check_car_s >  end_path_s - 30) && ((check_car_s - end_path_s) < ref_dist));
						}
					...
					if (lane == 1) {
						if ( d > 0 && d < 4 ) {
							// check if there is a car within +/- reference distance
							car_to_left |= (( check_car_s >  end_path_s - 30) && ((check_car_s - end_path_s) < ref_dist + 10));
						} else if ( d > 8 && d < 12 ) {
							// check if there is a car within +/- reference distance
							car_to_right |= (( check_car_s >  end_path_s - 30) && ((check_car_s - end_path_s) < ref_dist + 10));
						}
					}
			...
			// as reference state
			if (prev_size < 2) {
				// use two pints that make the path tangent to the car
				double prev_car_x = car_x - cos(car_yaw);
				double prev_car_y = car_y - sin(car_yaw);
				
				// use previous points to make sure path is tangent
			...
			{
				// redifine reference state as previous state
			...
			// Loop through all the points and make the first point in current time is
			// X = 0m, Y = 0m, Yaw = 0°
			...
			// Calculate how to break up spline spoints tso that we tarvel at our desired reference velocity
